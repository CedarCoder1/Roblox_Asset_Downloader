...
formatText = input("Which text format? (Normal, API): ").lower()
if formatText == "2" or formatText == "api":
    buildType = input("Build type? (Studio, Player): ").lower()
...
# New stuff for the version list.
#Todo: string manipulation/separation.
versionsToDownload = []

with open("versionsToDownload.txt", "r") as file:
    data = file.read()
    for line in data.splitlines():
        versionsToDownload.append(line) # Add line to table.
...
# Here to zip up the version when we are done downloading.
...
# Welcome to the loop of death!
def main_code(versionHash, buildType):
    # List Downloader
    if versionHash == "list":
        # RobloxApp
    ...
    # Windows
        ...
        # Player
        if buildType == "player" or buildType ==  "2" or buildType ==  "WindowsPlayer":
        ...
        # Studio
        elif buildType == "studio" or buildType == "1" or buildType ==  "Studio64":
            # rbxPkgManifest download + list maker.
            download = download_extra(downloadVersionSite + versionHash + "-rbxPkgManifest.txt")
            with open(os.path.join(extra_dir, download), "r") as manifest_file:
                manifest = manifest_file.read()
...
for line in versionsToDownload:
    if formatText == "1" or formatText == "normal":
        versionHash = line.split("version-")[1].split(" ")[0]
        print("Selected version hash: " + versionHash)
        buildType = line.split(" ")[1]
        print("Selected build type: " + buildType)
        main_code(versionHash, buildType)
    elif formatText == "2" or formatText == "api":
        versionHash = line.split("version-")[1].split(",")[0].replace('"', "")
        print("Selected version hash: " + versionHash)
        print("Selected build type: " + buildType)
        main_code(versionHash, buildType)
